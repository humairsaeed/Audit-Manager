  ...
  if (error || !audit) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="mt-2 text-lg font-medium text-slate-900 dark:text-slate-100">Audit not found</h3>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-4">
          <Link href="/audits" className="mt-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">
            <ArrowLeftIcon className="h-5 w-5" />
          </Link>
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">{audit.name}</h1>
      ...
      {canEdit && availableTransitions.length > 0 && (
        <div className="card p-4 bg-slate-50 dark:bg-slate-700/50">
          <div className="flex items-center justify-between">
            <span className="text-sm text-slate-600 dark:text-slate-300">Available actions:</span>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {(audit.scope || audit.objectives) && (
              <div className="card p-6">
                {audit.scope && (
                  <div className="mb-6">
                    <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-2">Scope</h2>
                    <p className="text-slate-700 dark:text-slate-300 whitespace-pre-wrap">{audit.scope}</p>
                  </div>
                )}
                {audit.objectives && (
                  <div>
                    <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-2">Objectives</h2>
                    <p className="text-slate-700 dark:text-slate-300 whitespace-pre-wrap">{audit.objectives}</p>
                  </div>
                )}
              </div>
            )}
                      ...
                      <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2">
                        <div
                          className={clsx('h-2 rounded-full', {
                            ...
                            'bg-slate-500': rating === 'INFORMATIONAL',
                          })}
              ...
              <dl className="space-y-4">
                {(audit.startDate || audit.endDate) && (
                  <div>
                    ...
                    <dd className="text-sm font-medium text-slate-900 dark:text-slate-100 mt-1">
      ...
      {activeTab === 'observations' && (
        <div className="card overflow-hidden">
          {observationsLoading ? (
            ...
            <div className="p-8 text-center">
              <p className="text-slate-500 dark:text-slate-400">No observations for this audit</p>
              {canAddObservation && audit.status === 'IN_PROGRESS' && (
                    ...
                    <tr key={obs.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                      <td className="px-6 py-4">
                        <Link href={`/observations/${obs.id}`}>
                          <p className="text-sm font-medium text-slate-900 dark:text-slate-100 hover:text-primary-600 dark:hover:text-primary-400">
                            {obs.title}
                          </p>
                      ...
                      <td className="px-6 py-4">
                        <span className={clsx('text-sm', isOverdue ? 'text-red-600 dark:text-red-400 font-medium' : 'text-slate-900 dark:text-slate-100')}>
      ...
      {activeTab === 'team' && (
        <div className="card p-6">
          <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-4">Audit Team</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {audit.leadAuditor && (
              <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                <p className="text-xs text-slate-500 dark:text-slate-400 uppercase mb-2">Lead Auditor</p>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400">{audit.leadAuditor.email}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {audit.teamMembers?.map((member: any) => (
              <div key={member.id} className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                <p className="text-xs text-slate-500 dark:text-slate-400 uppercase mb-2">Team Member</p>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-slate-400 dark:bg-slate-600 flex items-center justify-center">
                    <span className="text-white font-medium">
                      {member.firstName?.[0]}
                      {member.lastName?.[0]}
                    </span>
            ...
            {!audit.leadAuditor && (!audit.teamMembers || audit.teamMembers.length === 0) && (
              <p className="text-slate-500 dark:text-slate-400 col-span-full">No team members assigned</p>
            )}
          </div>
        </div>
      )}
